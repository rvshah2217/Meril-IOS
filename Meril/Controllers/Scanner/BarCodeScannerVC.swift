...
class BarCodeScannerVC: UIViewController {
    ...
    func setBackCamera() {
        ...
        do {
            ...
            GlobalFunctions.printToConsole(message: error.localizedDescription)//(error)
            return
        }
    }
    
    func failed() {
        let ac = UIAlertController(title: "Scanning not supported", message: "Your device does not support scanning a code from an item. Please use device with a camera.", preferredStyle: .alert)
